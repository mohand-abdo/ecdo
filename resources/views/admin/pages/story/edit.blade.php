                ...
                <div class="box-body">
                    ...
                    <form action="{{ route('admin.story.update',$story->id) }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control {{ $errors->has('title') ? 'has-error is-invalid' : ''  }}" name="title" placeholder="Place Enter Title" id="title_for" value="{{ old('title',$story->title) }}">
                             @error('title')
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control  {{ $errors->has('section_id') ? 'has-error is-invalid' : ''  }}" name="section_id" placeholder="Place Enter Section" id="section_for">
                                @foreach($sections as $section)
                                    <option @selected(old('section_id',$section->id) == $story->section_id) value="{{ $section->id }}">{{ $section->title }}</option>
                                @endforeach
                            </select>
                            @error('section_id')
                            <span style="color:red">{{$message}}</span>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="textarea {{ $errors->has('title_en') ? 'has-error is-invalid' : ''  }}" name="body" placeholder="Place Enter Description" style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;" id="title_en_for">{!! old('body',$story->body)  !!}</textarea>
                             @error('body')
                                <span style="color:red">{{$message}}</span>
                             @enderror
                         </div>
                         ...
                         <div class="form-group">
                            ...
                            <div class="input-group">
                                ...
                                <img id="blah" src="{{ asset('images/story/'.$story->image) }}" alt="no image" width="150" height="150" />
                                @error('image')
